...
## 🔗 СИСТЕМА ОБЪЕДИНЕНИЯ КЛИЕНТОВ (Client Merge)

### Концепция
Когда обнаруживается, что два клиента — один человек (совпадение телефона, WhatsApp, etc.), они объединяются.
...
|------|-----|----------|
| id | UUID | PK |
| master_client_id | UUID | Главный клиент (остаётся активным) |
| merged_client_id | UUID | Объединённый клиент (деактивируется) |
| merged_by | VARCHAR | ID оператора или "system" |
| merge_reason | TEXT | Причина: same_phone, same_whatsapp, manual, ai_detected |
| merged_at | TIMESTAMP | Дата объединения |
...
### Концепция
Система получения новых идентификаторов клиента через целевые действия ("приманки").

**Ключевой принцип:** Идентификаторы группируются в **классы эквивалентности**:
- Нет смысла получать WhatsApp, если есть телефон — это один класс
- Но Telegram, VK, Yandex — это новые классы
...
1. **identifier_classes** — классы идентификаторов
2. **identifier_class_members** — какие каналы входят в класс
3. **enrichment_routes** — маршруты (откуда → куда)
4. **enrichment_action_templates** — шаблоны приманок
5. **enrichment_tracking_codes** — tracking для отслеживания
6. **client_enrichment_history** — история предложений
...
4. Находим маршруты в недостающие классы
5. Фильтруем по cooldown
6. Выбираем лучший маршрут (priority × conversion)
7. Генерируем tracking_code
8. Вставляем приманку в ответ
9. При конверсии — фиксируем результат
```
...
## 📍 СИСТЕМА TOUCHPOINTS (Точек касания)

### Типы касаний

| Тип | Направление | Ожидаем ответ? | Участвует в mutual |
|-----|-------------|----------------|-------------------|
| inbound | Клиент → Нам | Да | Да |
| outbound | Мы → Клиент | Да | Да |
| promo | Мы → Клиент | Нет | Только с creates_entity |
| mutual | Двусторонний | — | Результат |

### Логика определения mutual:
- `inbound` + есть `outbound` за период → `mutual`
- `inbound` + `promo` + `creates_entity=true` → `mutual`
- `promo` → всегда остаётся `promo`

### Workflows:
- `BAT_Neo4j_Touchpoint_Register` — регистрация touchpoints
- `BAT_Appeal_Manager_v2` — с интеграцией touchpoints (inbound)
- `BAT_OUT_*_v2` — исходящие каналы с touchpoints
...
curl -X POST "https://n8n.n8nsrv.ru/webhook/neo4j/crud" \
  ...
  -d '{"operation": "create", "nodeType": "Client", "nodeId": "uuid", "properties": {...}}'
...
| HAS_PROBLEM | Device → Problem | Поломка устройства |
...
| ENRICHED_VIA | Client → Channel | Канал получен через enrichment |
| FROM/TO | Touchpoint → Client | Направление касания |

---

Это **полная карта** работы проекта Eldoleado (ранее BatteryCRM)! Каждое сообщение проходит через ~15-20 воркеров, с AI-обработкой, multi-tenant изоляцией и push-уведомлениями. Система включает:
- **Neo4j граф** для связей и контекста
- **Channel Enrichment** для обогащения профилей
- **Client Merge** для объединения дубликатов
- **Touchpoints** для отслеживания касаний
