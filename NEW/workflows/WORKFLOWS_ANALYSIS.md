...
> Total: 40 workflows (14 active, 26 inactive)
...
| Workflow | Method | Path | Status | Note |
|----------|--------|------|--------|------|
| ELO_In_WhatsApp | POST | /whatsapp-incoming | Active | |
| ELO_In_App | POST | /in-app | Active | |
| ELO_In_Telegram | POST | /telegram-in | Inactive | |
| ELO_In_VK | POST | /vk | Inactive | |
| ELO_In_Avito | POST | /avito | Inactive | |
| ELO_In_MAX | POST | /max | Inactive | |
| ELO_In_Phone | POST | /phone | Inactive | |
| ELO_In_Form | POST | /form | Inactive | |
...
| ELO_Client_Resolve | - | /elo-client-resolve | Active | Execute WF |
| ELO_API_Android_Auth | POST | /android/auth/login | Active | |
| ELO_API_Android_Dialogs | GET | /android/dialogs | Active | |
| ELO_API_Android_Messages | GET | /android/messages | Active | query params |
| ELO_API_Android_Send_Message | POST | /android-messages/... | Active | **PATH PARAM BUG** |
| ELO_API_Android_Normalize | POST | /android/dialogs/:id/normalize | Active | **PATH PARAM BUG** |
...
1. **PATH PARAMS NOT SUPPORTED** - n8n webhooks don't support `:param` in path
   - `ELO_API_Android_Normalize`: `/android/dialogs/:dialog_id/normalize` - BROKEN
   - `ELO_API_Android_Send_Message`: `/android-messages/android/dialogs/:dialog_id/messages` - BROKEN
   - **FIX:** Change to query params or body params (see Start.md Priorities 1-2)

2. **ELO_API_Android_Send_Message** - Does NOT call `ELO_Out_Router`
   - Message is saved to DB but not sent to client
   - Need to add HTTP call to `/webhook/elo-out-router`

3. **No FCM Push Workflow** - When new message arrives, no workflow pushes to Android
   - Need `ELO_Notify_Operator` workflow
   - Should be called after `ELO_Client_Resolve`

### High

4. **Input Pipeline Inactive** - `ELO_Input_Batcher` and `ELO_Input_Processor` are inactive
   - Messages pushed to Redis but not processed
   - Currently using direct flow (ELO_In_WhatsApp -> ELO_Client_Resolve)

5. **Operator assignment missing** - New dialogs don't get assigned_operator_id
   - See Start.md Priority 3 for fix

### Medium

6. **Incoming messages not saved** - `ELO_Client_Resolve` doesn't save to `elo_t_messages`
   - Patch created: `Client Contour/PATCH_ELO_Client_Resolve.md`

7. **Most Out channels inactive** - Only Router is active
   - ELO_Out_WhatsApp, ELO_Out_Telegram, etc. need activation

8. **AI Core is stub** - Only test stub active
