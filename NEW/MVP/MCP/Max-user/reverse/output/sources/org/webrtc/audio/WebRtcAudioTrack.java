...
public class WebRtcAudioTrack {
    ...
    private void logMainParameters() {
        ...
        StringBuilder sbK = wy1.k("AudioTrack: session ID: ", audioSessionId, ", channels: ", channelCount, ", sample rate: ");
        sbK.append(sampleRate);
        sbK.append(", max gain: ");
        sbK.append(maxVolume);
        ...
        if (!TRACK_LATEINIT) {
            try {
                ...
                if (this.audioTrack.getPlayState() != 3) {
                    reportWebRtcAudioTrackStartError(JavaAudioDeviceModule.AudioTrackStartErrorCode.AUDIO_TRACK_START_STATE_MISMATCH, ho7.f(this.audioTrack.getPlayState(), "AudioTrack.play failed - incorrect state :"));
                    releaseAudioResources();
                    return false;
                }
            } catch (IllegalStateException e) {
                reportWebRtcAudioTrackStartError(JavaAudioDeviceModule.AudioTrackStartErrorCode.AUDIO_TRACK_START_EXCEPTION, wy1.h("AudioTrack.play failed: ", e.getMessage()));
                releaseAudioResources();
                return false;
            }
        }
