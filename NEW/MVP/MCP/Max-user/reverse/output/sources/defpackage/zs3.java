package defpackage;

/* loaded from: classes.dex */
public final class zs3 implements p9e, h9e {
    public final long a;
    public final long b;
    public final int c;
    public final long d;
    public final int e;
    public final long f;
    public final boolean g;

    public zs3(long j, long j2, int i, int i2, boolean z) {
        this.a = j;
        this.b = j2;
        this.c = i2 == -1 ? 1 : i2;
        this.e = i;
        this.g = z;
        if (j == -1) {
            this.d = -1L;
            this.f = -9223372036854775807L;
        } else {
            long j3 = j - j2;
            this.d = j3;
            this.f = (Math.max(0L, j3) * 8000000) / i;
        }
    }

    @Override // defpackage.p9e
    public final long a() {
        return -1L;
    }

    @Override // defpackage.p9e
    public final long b(long j) {
        return (Math.max(0L, j - this.b) * 8000000) / this.e;
    }

    @Override // defpackage.h9e
    public final boolean c() {
        return this.d != -1 || this.g;
    }

    @Override // defpackage.h9e
    public final f9e e(long j) {
        long j2 = this.d;
        long j3 = this.b;
        if (j2 == -1 && !this.g) {
            l9e l9eVar = new l9e(0L, j3);
            return new f9e(l9eVar, l9eVar);
        }
        int i = this.e;
        long j4 = this.c;
        long jMin = (((i * j) / 8000000) / j4) * j4;
        if (j2 != -1) {
            jMin = Math.min(jMin, j2 - j4);
        }
        long jMax = Math.max(jMin, 0L) + j3;
        long jMax2 = (Math.max(0L, jMax - j3) * 8000000) / i;
        l9e l9eVar2 = new l9e(jMax2, jMax);
        if (j2 != -1 && jMax2 < j) {
            long j5 = jMax + j4;
            if (j5 < this.a) {
                return new f9e(l9eVar2, new l9e((Math.max(0L, j5 - j3) * 8000000) / i, j5));
            }
        }
        return new f9e(l9eVar2, l9eVar2);
    }

    @Override // defpackage.h9e
    public final long f() {
        return this.f;
    }
}
