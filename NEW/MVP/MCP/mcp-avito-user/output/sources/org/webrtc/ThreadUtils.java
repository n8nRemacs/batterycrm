...
public class ThreadUtils {
        ...
        handler.post(new Runnable() { // from class: org.webrtc.ThreadUtils.3
            ...
            public void run() {
                try {
                    ...
                    c1CaughtException.f422267e = e13;
                }
                countDownLatch.countDown();
            }
        });
        awaitUninterruptibly(countDownLatch);
        if (c1CaughtException.f422267e == null) {
            return c1Result.value;
        }
        RuntimeException runtimeException = new RuntimeException(c1CaughtException.f422267e);
        runtimeException.setStackTrace(concatStackTraces(c1CaughtException.f422267e.getStackTrace(), runtimeException.getStackTrace()));
        throw runtimeException;
    }

    public static boolean joinUninterruptibly(Thread thread, long j12) throws InterruptedException {
        long jElapsedRealtime = SystemClock.elapsedRealtime();
        boolean z12 = false;
        long jElapsedRealtime2 = j12;
        while (jElapsedRealtime2 > 0) {
            try {
                thread.join(jElapsedRealtime2);
                break;
            } catch (InterruptedException unused) {
                jElapsedRealtime2 = j12 - (SystemClock.elapsedRealtime() - jElapsedRealtime);
                z12 = true;
            }
        }
        if (z12) {
    ...
    public static boolean awaitUninterruptibly(CountDownLatch countDownLatch, long j12) throws InterruptedException {
        long jElapsedRealtime = SystemClock.elapsedRealtime();
