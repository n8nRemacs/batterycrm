...
public final class MacKeyTemplates {
    ...
    public static KeyTemplate createHmacKeyTemplate(int i12, int i13, HashType hashType) {
        return KeyTemplate.newBuilder().setValue(HmacKeyFormat.newBuilder().setParams(HmacParams.newBuilder().setHash(hashType).setTagSize(i13).build()).setKeySize(i12).build().toByteString()).setTypeUrl(new HmacKeyManager().getKeyType()).setOutputPrefixType(OutputPrefixType.TINK).build();
    }
}
