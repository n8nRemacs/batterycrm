...
final class ByteBufferWriter {
    ...
    private static byte[] getOrCreateBuffer(int i12) {
        int iMax = Math.max(i12, MIN_CACHED_BUFFER_SIZE);
    ...
    private static boolean needToReallocate(int i12, int i13) {
        return i13 < i12 && ((float) i13) < ((float) i12) * BUFFER_REALLOCATION_THRESHOLD;
    }
    ...
    private static boolean writeToChannel(ByteBuffer byteBuffer, OutputStream outputStream) throws IOException {
        ...
        long j12 = CHANNEL_FIELD_OFFSET;
        if (j12 < 0 || !FILE_OUTPUT_STREAM_CLASS.isInstance(outputStream)) {
            return false;
        }
        try {
            writableByteChannel = (WritableByteChannel) UnsafeUtil.getObject(outputStream, j12);
