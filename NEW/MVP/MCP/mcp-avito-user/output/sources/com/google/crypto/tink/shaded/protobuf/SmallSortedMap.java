...
class SmallSortedMap<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    ...
    public class DescendingEntryIterator implements Iterator<Map.Entry<K, V>> {
        ...
        public boolean hasNext() {
            int i12 = this.pos;
            return (i12 > 0 && i12 <= SmallSortedMap.this.entryList.size()) || getOverflowIterator().hasNext();
        }
    ...
    public class EntryIterator implements Iterator<Map.Entry<K, V>> {
        ...
        public void remove() {
            ...
            int i12 = this.pos;
            this.pos = i12 - 1;
            smallSortedMap.removeArrayEntryAt(i12);
        }
        ...
        public Map.Entry<K, V> next() {
            ...
            int i12 = this.pos + 1;
            this.pos = i12;
            return i12 < SmallSortedMap.this.entryList.size() ? (Map.Entry) SmallSortedMap.this.entryList.get(this.pos) : getOverflowIterator().next();
        }
    }
    ...
    */
    ...
    public static <FieldDescriptorType extends FieldSet.FieldDescriptorLite<FieldDescriptorType>> SmallSortedMap<FieldDescriptorType, Object> newFieldMap(int i12) {
        return (SmallSortedMap<FieldDescriptorType, Object>) new SmallSortedMap<FieldDescriptorType, Object>(i12) { // from class: com.google.crypto.tink.shaded.protobuf.SmallSortedMap.1
    ...
    public static <K extends Comparable<K>, V> SmallSortedMap<K, V> newInstanceForTest(int i12) {
        return new SmallSortedMap<>(i12);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public V removeArrayEntryAt(int i12) {
        checkMutable();
        V value = this.entryList.remove(i12).getValue();
    ...
    public boolean equals(Object obj) {
        ...
        for (int i12 = 0; i12 < numArrayEntries; i12++) {
            if (!getArrayEntryAt(i12).equals(smallSortedMap.getArrayEntryAt(i12))) {
                return false;
            }
        }
    ...
    public Map.Entry<K, V> getArrayEntryAt(int i12) {
        return this.entryList.get(i12);
    }
    ...
    public int hashCode() {
        ...
        int iHashCode = 0;
        for (int i12 = 0; i12 < numArrayEntries; i12++) {
            iHashCode += this.entryList.get(i12).hashCode();
        }
    ...
    public V put(K k12, V v12) {
        checkMutable();
        int iBinarySearchInArray = binarySearchInArray(k12);
        if (iBinarySearchInArray >= 0) {
            return this.entryList.get(iBinarySearchInArray).setValue(v12);
        }
        ensureEntryArrayMutable();
        int i12 = -(iBinarySearchInArray + 1);
        if (i12 >= this.maxArraySize) {
            return getOverflowEntriesMutable().put(k12, v12);
        }
        int size = this.entryList.size();
        int i13 = this.maxArraySize;
        if (size == i13) {
            SmallSortedMap<K, V>.Entry entryRemove = this.entryList.remove(i13 - 1);
            getOverflowEntriesMutable().put(entryRemove.getKey(), entryRemove.getValue());
        }
        this.entryList.add(i12, new Entry(k12, v12));
        return null;
    }
}
