...
public final class PrfKeyTemplates {
    ...
    private static KeyTemplate createHmacTemplate(int i12, HashType hashType) {
        return KeyTemplate.newBuilder().setTypeUrl(new HmacPrfKeyManager().getKeyType()).setValue(HmacPrfKeyFormat.newBuilder().setParams(HmacPrfParams.newBuilder().setHash(hashType).build()).setKeySize(i12).build().toByteString()).setOutputPrefixType(OutputPrefixType.RAW).build();
    }
}
