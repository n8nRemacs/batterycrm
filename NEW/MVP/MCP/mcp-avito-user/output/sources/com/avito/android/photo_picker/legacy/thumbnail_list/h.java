package com.avito.android.photo_picker.legacy.thumbnail_list;

import java.util.Iterator;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;

/* compiled from: ThumbnailItemPresenter.kt */
@Metadata(d1 = {"\u0000\u000e\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0005\u001a\u00020\u00022\u0006\u0010\u0001\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0003\u0010\u0004"}, d2 = {"Lcom/avito/android/device_orientation/c;", "it", "Lkotlin/G0;", "invoke", "(Lcom/avito/android/device_orientation/c;)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes14.dex */
final class h extends N implements Y41.l<com.avito.android.device_orientation.c, G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ i f219893l;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public h(i iVar) {
        super(1);
        this.f219893l = iVar;
    }

    @Override // Y41.l
    public final G0 invoke(com.avito.android.device_orientation.c cVar) {
        com.avito.android.device_orientation.c cVar2 = cVar;
        i iVar = this.f219893l;
        iVar.f219896d = cVar2;
        Iterator<j> it = iVar.f219898f.iterator();
        while (it.hasNext()) {
            it.next().FW(cVar2, true);
        }
        return G0.f406611a;
    }
}
