package com.avito.android.serp.adapter.actions_horizontal_block;

import com.avito.android.deep_linking.links.DeepLink;
import com.avito.android.deeplink_handler.handler.b;
import com.avito.android.serp.adapter.actions_horizontal_block.Action;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;
import kotlin.jvm.internal.s0;

/* compiled from: ActionsHorizontalBlockItemPresenter.kt */
@s0
@Metadata(d1 = {"\u0000\b\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0003\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0001\u0010\u0002"}, d2 = {"Lkotlin/G0;", "invoke", "()V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes3.dex */
final class p extends N implements Y41.a<G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ Action f268690l;

    /* renamed from: m, reason: collision with root package name */
    public final /* synthetic */ g f268691m;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public p(Action action, g gVar) {
        super(0);
        this.f268690l = action;
        this.f268691m = gVar;
    }

    @Override // Y41.a
    public final G0 invoke() {
        DeepLink deepLink = ((Action.Predefined) this.f268690l).f268644g;
        if (deepLink != null) {
            b.a.a(this.f268691m.f268675d, deepLink, null, null, 6);
        }
        return G0.f406611a;
    }
}
