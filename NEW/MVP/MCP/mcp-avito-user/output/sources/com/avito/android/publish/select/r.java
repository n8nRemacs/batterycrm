package com.avito.android.publish.select;

import com.avito.android.remote.model.category_parameters.SelectParameter;
import kotlin.Metadata;

/* compiled from: SelectViewModel.kt */
@Metadata(d1 = {"\u0000\u000e\n\u0002\u0010\u000e\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0005\u001a\u00020\u00022\u0006\u0010\u0001\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0003\u0010\u0004"}, d2 = {"", "it", "Lkotlin/G0;", "accept", "(Ljava/lang/String;)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes16.dex */
final class r<T> implements l41.g {

    /* renamed from: b, reason: collision with root package name */
    public final /* synthetic */ SelectViewModel f242710b;

    public r(SelectViewModel selectViewModel) {
        this.f242710b = selectViewModel;
    }

    @Override // l41.g
    public final void accept(Object obj) {
        String str = (String) obj;
        SelectViewModel selectViewModel = this.f242710b;
        SelectParameter selectParameter = selectViewModel.f242641P;
        if (selectParameter == null) {
            selectParameter = null;
        }
        selectParameter.setValue(str);
        selectViewModel.f242640O.xe(null);
    }
}
