package com.avito.android.crm_paid_cvs.view.list_items.favorite_item;

import com.avito.android.crm_paid_cvs.dto.CvItem;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;

/* compiled from: FavoriteItemPresenter.kt */
@Metadata(d1 = {"\u0000\b\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0003\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0001\u0010\u0002"}, d2 = {"Lkotlin/G0;", "invoke", "()V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes12.dex */
final class j extends N implements Y41.a<G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ l f130896l;

    /* renamed from: m, reason: collision with root package name */
    public final /* synthetic */ CvItem f130897m;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public j(CvItem cvItem, l lVar) {
        super(0);
        this.f130896l = lVar;
        this.f130897m = cvItem;
    }

    @Override // Y41.a
    public final G0 invoke() {
        this.f130896l.f130901c.invoke(this.f130897m);
        return G0.f406611a;
    }
}
