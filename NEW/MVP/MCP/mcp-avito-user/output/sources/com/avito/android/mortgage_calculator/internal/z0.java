package com.avito.android.mortgage_calculator.internal;

import kotlin.Metadata;

/* compiled from: MortgageCalculatorScreen.kt */
@Metadata(d1 = {"\u0000\u000e\n\u0002\u0010\u0007\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0005\u001a\u00020\u00022\u0006\u0010\u0001\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0003\u0010\u0004"}, d2 = {"", "it", "Lkotlin/G0;", "invoke", "(F)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes15.dex */
final class z0 extends kotlin.jvm.internal.N implements Y41.l<Float, kotlin.G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ Y41.l<Float, kotlin.G0> f204831l;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    /* JADX WARN: Multi-variable type inference failed */
    public z0(Y41.l<? super Float, kotlin.G0> lVar) {
        super(1);
        this.f204831l = lVar;
    }

    @Override // Y41.l
    public final kotlin.G0 invoke(Float f12) {
        float fFloatValue = f12.floatValue();
        Y41.l<Float, kotlin.G0> lVar = this.f204831l;
        if (lVar != null) {
            lVar.invoke(Float.valueOf(fFloatValue));
        }
        return kotlin.G0.f406611a;
    }
}
