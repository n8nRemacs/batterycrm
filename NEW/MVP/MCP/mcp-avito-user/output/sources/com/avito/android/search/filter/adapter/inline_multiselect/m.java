package com.avito.android.search.filter.adapter.inline_multiselect;

import Y41.p;
import com.avito.android.beduin.common.component.cart_item.BeduinCartItemModel;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;

/* compiled from: InlineMultiselectItemView.kt */
@Metadata(d1 = {"\u0000\u000e\n\u0002\u0010\u000b\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0005\u001a\u00020\u00022\u0006\u0010\u0001\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0003\u0010\u0004"}, d2 = {"", BeduinCartItemModel.CHECKED_STRING, "Lkotlin/G0;", "invoke", "(Z)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes3.dex */
final class m extends N implements Y41.l<Boolean, G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ p<Cp0.e, Boolean, G0> f262379l;

    /* renamed from: m, reason: collision with root package name */
    public final /* synthetic */ Cp0.e f262380m;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    /* JADX WARN: Multi-variable type inference failed */
    public m(p<? super Cp0.e, ? super Boolean, G0> pVar, Cp0.e eVar) {
        super(1);
        this.f262379l = pVar;
        this.f262380m = eVar;
    }

    @Override // Y41.l
    public final G0 invoke(Boolean bool) {
        Boolean bool2 = bool;
        bool2.booleanValue();
        this.f262379l.invoke(this.f262380m, bool2);
        return G0.f406611a;
    }
}
