package com.avito.android.cpx_promo.priceinput;

import android.text.Editable;
import android.text.TextWatcher;
import com.avito.android.cpx_promo.priceinput.CpxPromoPriceInputFragment;
import com.avito.android.lib.design.input.Input;
import is.InterfaceC42096a;
import kotlin.Metadata;
import kotlin.jvm.internal.L;
import kotlin.jvm.internal.s0;

/* compiled from: InputExtensions.kt */
@s0
@Metadata(d1 = {"\u0000\u000b\n\u0000\n\u0002\u0018\u0002\n\u0000*\u0001\u0000\b\n\u0018\u00002\u00020\u0001¨\u0006\u0002¸\u0006\u0000"}, d2 = {"com/avito/android/lib/design/input/l", "Landroid/text/TextWatcher;", "_design-modules_components"}, k = 1, mv = {1, 9, 0}, xi = 48)
/* loaded from: classes12.dex */
public final class e implements TextWatcher {

    /* renamed from: b, reason: collision with root package name */
    @Y61.k
    public String f127315b;

    /* renamed from: c, reason: collision with root package name */
    public final /* synthetic */ Input f127316c;

    /* renamed from: d, reason: collision with root package name */
    public final /* synthetic */ CpxPromoPriceInputFragment f127317d;

    public e(Input input, CpxPromoPriceInputFragment cpxPromoPriceInputFragment) {
        this.f127316c = input;
        this.f127317d = cpxPromoPriceInputFragment;
        this.f127315b = input.getDeformattedText();
    }

    @Override // android.text.TextWatcher
    public final void afterTextChanged(@Y61.l Editable editable) {
        String deformattedText = this.f127316c.getDeformattedText();
        if (L.f(deformattedText, this.f127315b)) {
            return;
        }
        CpxPromoPriceInputFragment.a aVar = CpxPromoPriceInputFragment.f127271s0;
        ((l) this.f127317d.f127273i0.getValue()).accept(new InterfaceC42096a.C11539a(deformattedText));
        this.f127315b = deformattedText;
    }

    @Override // android.text.TextWatcher
    public final void beforeTextChanged(@Y61.l CharSequence charSequence, int i12, int i13, int i14) {
    }

    @Override // android.text.TextWatcher
    public final void onTextChanged(@Y61.l CharSequence charSequence, int i12, int i13, int i14) {
    }
}
