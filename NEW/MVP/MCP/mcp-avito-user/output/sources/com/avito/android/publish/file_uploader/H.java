package com.avito.android.publish.file_uploader;

import com.avito.android.remote.model.category_parameters.FileUploadValidationRule;
import fe0.d;
import kotlin.Metadata;
import kotlin.jvm.internal.s0;

/* compiled from: FileValidator.kt */
@s0
@Metadata(d1 = {"\u0000\u0002\n\u0000¨\u0006\u0000"}, d2 = {"_avito_publish_impl"}, k = 2, mv = {1, 9, 0}, xi = 48)
/* loaded from: classes16.dex */
public final class H {
    public static final fe0.d a(FileUploadValidationRule fileUploadValidationRule, Y41.a<Boolean> aVar) {
        if (aVar.invoke().booleanValue()) {
            return d.c.f396010b;
        }
        String message = fileUploadValidationRule.getMessage();
        if (message == null) {
            message = "";
        }
        return new d.b(message);
    }
}
