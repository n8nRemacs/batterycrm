package com.avito.android.tariff.cpa.prepaid_expense;

import com.avito.android.tariff.cpa.prepaid_expense.PublicationAdvanceFragment;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;

/* compiled from: PublicationAdvanceFragment.kt */
@Metadata(d1 = {"\u0000\u0010\n\u0002\u0010\u000e\n\u0002\b\u0002\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0006\u001a\u00020\u00032\u000e\u0010\u0002\u001a\n \u0001*\u0004\u0018\u00010\u00000\u0000H\n¢\u0006\u0004\b\u0004\u0010\u0005"}, d2 = {"", "kotlin.jvm.PlatformType", "it", "Lkotlin/G0;", "invoke", "(Ljava/lang/String;)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes4.dex */
final class g extends N implements Y41.l<String, G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ PublicationAdvanceFragment f295009l;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public g(PublicationAdvanceFragment publicationAdvanceFragment) {
        super(1);
        this.f295009l = publicationAdvanceFragment;
    }

    @Override // Y41.l
    public final G0 invoke(String str) {
        PublicationAdvanceFragment.a aVar = PublicationAdvanceFragment.f294939y0;
        com.avito.android.lib.design.button.b.a(this.f295009l.q5(), str, false);
        return G0.f406611a;
    }
}
