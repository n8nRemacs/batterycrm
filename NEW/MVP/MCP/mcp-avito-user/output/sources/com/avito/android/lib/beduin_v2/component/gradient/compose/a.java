package com.avito.android.lib.beduin_v2.component.gradient.compose;

import Y41.l;
import androidx.compose.ui.graphics.C22322z0;
import androidx.compose.ui.graphics.drawscope.g;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;
import okhttp3.internal.ws.WebSocketProtocol;

/* compiled from: GradientModifier.kt */
@Metadata(d1 = {"\u0000\f\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0004\u001a\u00020\u0001*\u00020\u0000H\n¢\u0006\u0004\b\u0002\u0010\u0003"}, d2 = {"Landroidx/compose/ui/graphics/drawscope/g;", "Lkotlin/G0;", "invoke", "(Landroidx/compose/ui/graphics/drawscope/g;)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes14.dex */
final class a extends N implements l<g, G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ C22322z0 f175606l;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public a(C22322z0 c22322z0) {
        super(1);
        this.f175606l = c22322z0;
    }

    @Override // Y41.l
    public final G0 invoke(g gVar) {
        g.e0(gVar, this.f175606l, 0L, 0L, 0.0f, null, null, WebSocketProtocol.PAYLOAD_SHORT);
        return G0.f406611a;
    }
}
