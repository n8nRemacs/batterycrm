package com.avito.android.advert_stats;

import com.avito.android.advert_stats.AdvertStatsFragment;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;

/* compiled from: AdvertStatsFragment.kt */
@Metadata(d1 = {"\u0000\b\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0003\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0001\u0010\u0002"}, d2 = {"Lkotlin/G0;", "invoke", "()V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes10.dex */
final class i extends N implements Y41.a<G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ AdvertStatsFragment f86672l;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public i(AdvertStatsFragment advertStatsFragment) {
        super(0);
        this.f86672l = advertStatsFragment;
    }

    @Override // Y41.a
    public final G0 invoke() {
        AdvertStatsFragment.a aVar = AdvertStatsFragment.f85994z0;
        r rVarQ5 = this.f86672l.q5();
        rVarQ5.le(rVarQ5.f86844E);
        return G0.f406611a;
    }
}
