package com.avito.android.advert.deeplinks;

import com.avito.android.deep_linking.links.AddItemToCartLink;
import com.avito.android.lib.beduin_v2.repository.domain.cart_items.model.CartItemInfo;
import kotlin.Metadata;

/* compiled from: AddItemToCartDeepLinkHandler.kt */
@Metadata(d1 = {"\u0000\u000e\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0005\u001a\u00020\u00022\u0006\u0010\u0001\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0003\u0010\u0004"}, d2 = {"Lcom/avito/android/remote/safedeal/a;", "it", "Lkotlin/G0;", "accept", "(Lcom/avito/android/remote/safedeal/a;)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes10.dex */
final class c<T> implements l41.g {

    /* renamed from: b, reason: collision with root package name */
    public final /* synthetic */ a f68897b;

    /* renamed from: c, reason: collision with root package name */
    public final /* synthetic */ AddItemToCartLink f68898c;

    public c(a aVar, AddItemToCartLink addItemToCartLink) {
        this.f68897b = aVar;
        this.f68898c = addItemToCartLink;
    }

    @Override // l41.g
    public final void accept(Object obj) {
        a aVar = this.f68897b;
        aVar.f68889m.b(this.f68898c.f132936b, new CartItemInfo(1, null, 2, null));
    }
}
