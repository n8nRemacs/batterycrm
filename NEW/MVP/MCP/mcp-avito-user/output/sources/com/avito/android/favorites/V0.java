package com.avito.android.favorites;

import android.view.ViewGroup;
import com.avito.android.progress_info_toast_bar.ProgressInfoToastBar;
import com.avito.android.progress_info_toast_bar.ProgressInfoToastBarData;
import kotlin.Metadata;

/* compiled from: FavoritesListPresenter.kt */
@Metadata(d1 = {"\u0000\u000e\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0005\u001a\u00020\u00022\u0006\u0010\u0001\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0003\u0010\u0004"}, d2 = {"Lcom/avito/android/progress_info_toast_bar/ProgressInfoToastBarData;", "it", "Lkotlin/G0;", "accept", "(Lcom/avito/android/progress_info_toast_bar/ProgressInfoToastBarData;)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes13.dex */
final class V0<T> implements l41.g {

    /* renamed from: b, reason: collision with root package name */
    public final /* synthetic */ C30684r0 f156334b;

    public V0(C30684r0 c30684r0) {
        this.f156334b = c30684r0;
    }

    @Override // l41.g
    public final void accept(Object obj) {
        ProgressInfoToastBarData progressInfoToastBarData = (ProgressInfoToastBarData) obj;
        x1 x1Var = this.f156334b.f157454Y;
        if (x1Var != null) {
            ProgressInfoToastBar progressInfoToastBar = x1Var.f157618v;
            if (progressInfoToastBar != null) {
                progressInfoToastBar.dismiss();
            }
            ViewGroup viewGroup = x1Var.f157598b;
            com.avito.android.progress_info_toast_bar.d dVarA = x1Var.f157605i.a(viewGroup.getContext(), progressInfoToastBarData, null);
            dVarA.f231502d = viewGroup;
            dVarA.a();
            x1Var.f157618v = dVarA;
        }
    }
}
