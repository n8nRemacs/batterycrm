package com.avito.android.image_loader.glide.utils;

import android.net.Uri;
import android.widget.ImageView;
import com.avito.android.image_loader.ImageRequest;
import com.avito.android.image_loader.m;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;

/* compiled from: ImageViews.kt */
@Metadata(d1 = {"\u0000\b\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0003\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0001\u0010\u0002"}, d2 = {"Lkotlin/G0;", "invoke", "()V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes14.dex */
final class a extends N implements Y41.a<G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ ImageView f169552l;

    /* renamed from: m, reason: collision with root package name */
    public final /* synthetic */ m f169553m;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public a(ImageView imageView, m mVar) {
        super(0);
        this.f169552l = imageView;
        this.f169553m = mVar;
    }

    @Override // Y41.a
    public final G0 invoke() {
        m mVar = this.f169553m;
        ImageView imageView = this.f169552l;
        Uri uriA = mVar.a(imageView);
        ImageRequest.a aVar = new ImageRequest.a(new DM.a(imageView));
        aVar.f(uriA);
        aVar.f169485c = 0;
        aVar.c();
        return G0.f406611a;
    }
}
