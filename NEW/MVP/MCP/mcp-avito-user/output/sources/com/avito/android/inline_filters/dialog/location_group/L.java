package com.avito.android.inline_filters.dialog.location_group;

import com.avito.android.remote.model.search.InlineFilterValue;
import kotlin.G0;
import kotlin.Metadata;

/* compiled from: LocationGroupFilterView.kt */
@Metadata(d1 = {"\u0000\u000e\n\u0002\u0010\b\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0005\u001a\u00020\u00022\u0006\u0010\u0001\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0003\u0010\u0004"}, d2 = {"", "position", "Lkotlin/G0;", "invoke", "(I)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes14.dex */
final class L extends kotlin.jvm.internal.N implements Y41.l<Integer, G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ Y41.p<Integer, InlineFilterValue, G0> f171691l;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    /* JADX WARN: Multi-variable type inference failed */
    public L(Y41.p<? super Integer, ? super InlineFilterValue, G0> pVar) {
        super(1);
        this.f171691l = pVar;
    }

    @Override // Y41.l
    public final G0 invoke(Integer num) {
        this.f171691l.invoke(Integer.valueOf(num.intValue()), null);
        return G0.f406611a;
    }
}
