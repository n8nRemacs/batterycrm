package com.avito.beduin.v2.component.row.android_view;

import Y61.k;
import android.content.res.Resources;
import com.avito.beduin.v2.component.common.VerticalAlignment;
import kotlin.Metadata;

/* compiled from: RowComponent.kt */
@Metadata(d1 = {"\u0000\u0002\n\u0000¨\u0006\u0000"}, d2 = {"android-view_release"}, k = 2, mv = {1, 9, 0}, xi = 48)
/* loaded from: classes5.dex */
public final class e {

    /* compiled from: RowComponent.kt */
    @Metadata(k = 3, mv = {1, 9, 0}, xi = 48)
    public /* synthetic */ class a {
        static {
            int[] iArr = new int[VerticalAlignment.values().length];
            try {
                iArr[0] = 1;
            } catch (NoSuchFieldError unused) {
            }
            try {
                VerticalAlignment verticalAlignment = VerticalAlignment.f335668b;
                iArr[1] = 2;
            } catch (NoSuchFieldError unused2) {
            }
            try {
                VerticalAlignment verticalAlignment2 = VerticalAlignment.f335668b;
                iArr[2] = 3;
            } catch (NoSuchFieldError unused3) {
            }
        }
    }

    public static final int a(@k Resources resources, int i12) {
        if (i12 == -2) {
            return -2;
        }
        if (i12 != -1) {
            return ET0.d.b(resources, i12);
        }
        return -1;
    }
}
