package com.avito.android.messenger.blacklist_reasons.list_items;

import com.avito.android.messenger.blacklist_reasons.list_items.a;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;

/* compiled from: ReasonItemBlueprint.kt */
@Metadata(d1 = {"\u0000\b\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0003\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0001\u0010\u0002"}, d2 = {"Lkotlin/G0;", "invoke", "()V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes15.dex */
final class j extends N implements Y41.a<G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ k f186630l;

    /* renamed from: m, reason: collision with root package name */
    public final /* synthetic */ a.C5476a f186631m;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public j(k kVar, a.C5476a c5476a) {
        super(0);
        this.f186630l = kVar;
        this.f186631m = c5476a;
    }

    @Override // Y41.a
    public final G0 invoke() {
        this.f186630l.f186632b.kc(this.f186631m.f186614b);
        return G0.f406611a;
    }
}
