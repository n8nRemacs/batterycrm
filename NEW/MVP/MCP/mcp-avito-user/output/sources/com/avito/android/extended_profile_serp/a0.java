package com.avito.android.extended_profile_serp;

import bB.InterfaceC25487a;
import com.avito.android.extended_profile_serp.ExtendedProfileSerpFragment;
import kotlin.G0;
import kotlin.Metadata;

/* compiled from: ExtendedProfileSerpView.kt */
@Metadata(d1 = {"\u0000\b\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0003\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0001\u0010\u0002"}, d2 = {"Lkotlin/G0;", "invoke", "()V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes13.dex */
final class a0 extends kotlin.jvm.internal.N implements Y41.a<G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ F f152591l;

    /* renamed from: m, reason: collision with root package name */
    public final /* synthetic */ InterfaceC25487a.l f152592m;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public a0(F f12, InterfaceC25487a.l lVar) {
        super(0);
        this.f152591l = f12;
        this.f152592m = lVar;
    }

    @Override // Y41.a
    public final G0 invoke() {
        ((ExtendedProfileSerpFragment.e) this.f152591l.f152526c).invoke(this.f152592m);
        return G0.f406611a;
    }
}
