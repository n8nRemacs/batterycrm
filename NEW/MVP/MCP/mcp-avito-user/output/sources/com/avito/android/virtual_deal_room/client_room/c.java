package com.avito.android.virtual_deal_room.client_room;

import EN0.a;
import com.avito.android.virtual_deal_room.client_room.ClientRoomFragment;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;

/* compiled from: ClientRoomFragment.kt */
@Metadata(d1 = {"\u0000\b\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0003\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0001\u0010\u0002"}, d2 = {"Lkotlin/G0;", "invoke", "()V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes5.dex */
final class c extends N implements Y41.a<G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ ClientRoomFragment f326264l;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public c(ClientRoomFragment clientRoomFragment) {
        super(0);
        this.f326264l = clientRoomFragment;
    }

    @Override // Y41.a
    public final G0 invoke() {
        ClientRoomFragment.a aVar = ClientRoomFragment.f326248u0;
        this.f326264l.q5().accept(a.c.f3947a);
        return G0.f406611a;
    }
}
