package com.avito.android.tariff_cpt.configure.migration;

import com.avito.android.tariff_cpt.configure.migration.CptMigrationFragment;
import com.avito.android.tariff_cpt.configure.migration.viewmodel.h;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;

/* compiled from: CptMigrationFragment.kt */
@Metadata(d1 = {"\u0000\b\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0003\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0001\u0010\u0002"}, d2 = {"Lkotlin/G0;", "invoke", "()V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes4.dex */
final class b extends N implements Y41.a<G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ CptMigrationFragment f298015l;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public b(CptMigrationFragment cptMigrationFragment) {
        super(0);
        this.f298015l = cptMigrationFragment;
    }

    @Override // Y41.a
    public final G0 invoke() {
        CptMigrationFragment.a aVar = CptMigrationFragment.f297998v0;
        ((h) this.f298015l.f298003r0.getValue()).O();
        return G0.f406611a;
    }
}
