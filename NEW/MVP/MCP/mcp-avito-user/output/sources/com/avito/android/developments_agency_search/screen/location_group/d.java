package com.avito.android.developments_agency_search.screen.location_group;

import Iw.InterfaceC14163a;
import com.avito.android.developments_agency_search.screen.location_group.LocationGroupDialogFragment;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;
import oi0.C44783b;

/* compiled from: LocationGroupDialogFragment.kt */
@Metadata(d1 = {"\u0000\u000e\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0005\u001a\u00020\u00022\u0006\u0010\u0001\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0003\u0010\u0004"}, d2 = {"Loi0/b;", "location", "Lkotlin/G0;", "invoke", "(Loi0/b;)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes13.dex */
final class d extends N implements Y41.l<C44783b, G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ LocationGroupDialogFragment f138080l;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public d(LocationGroupDialogFragment locationGroupDialogFragment) {
        super(1);
        this.f138080l = locationGroupDialogFragment;
    }

    @Override // Y41.l
    public final G0 invoke(C44783b c44783b) {
        LocationGroupDialogFragment.a aVar = LocationGroupDialogFragment.f138049q0;
        this.f138080l.f5().accept(new InterfaceC14163a.d(c44783b));
        return G0.f406611a;
    }
}
