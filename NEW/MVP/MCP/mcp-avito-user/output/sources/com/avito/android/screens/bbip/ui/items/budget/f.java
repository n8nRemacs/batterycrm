package com.avito.android.screens.bbip.ui.items.budget;

import com.avito.android.screens.bbip.ui.items.budget.b;
import java.util.Iterator;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.L;
import kotlin.jvm.internal.N;
import kotlin.jvm.internal.s0;

/* compiled from: BudgetItemPresenter.kt */
@s0
@Metadata(d1 = {"\u0000\u000e\n\u0002\u0010\u000e\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0005\u001a\u00020\u00022\u0006\u0010\u0001\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0003\u0010\u0004"}, d2 = {"", "price", "Lkotlin/G0;", "invoke", "(Ljava/lang/String;)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes3.dex */
final class f extends N implements Y41.l<String, G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ b f260318l;

    /* renamed from: m, reason: collision with root package name */
    public final /* synthetic */ g f260319m;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public f(b bVar, g gVar) {
        super(1);
        this.f260318l = bVar;
        this.f260319m = gVar;
    }

    @Override // Y41.l
    public final G0 invoke(String str) {
        Object next;
        String str2 = str;
        Iterator<T> it = this.f260318l.f260306d.iterator();
        while (true) {
            if (!it.hasNext()) {
                next = null;
                break;
            }
            next = it.next();
            if (L.f(((b.a) next).f260307a, str2)) {
                break;
            }
        }
        b.a aVar = (b.a) next;
        if (aVar != null) {
            this.f260319m.f260320b.onNext(aVar);
        }
        return G0.f406611a;
    }
}
