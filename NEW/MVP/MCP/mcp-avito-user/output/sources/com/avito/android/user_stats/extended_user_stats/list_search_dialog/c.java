package com.avito.android.user_stats.extended_user_stats.list_search_dialog;

import Y41.l;
import androidx.recyclerview.widget.RecyclerView;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;

/* compiled from: ListSearchDialogView.kt */
@Metadata(d1 = {"\u0000\u000e\n\u0002\u0010\u000e\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0005\u001a\u00020\u00022\u0006\u0010\u0001\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0003\u0010\u0004"}, d2 = {"", "it", "Lkotlin/G0;", "invoke", "(Ljava/lang/String;)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes5.dex */
final class c extends N implements l<String, G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ d f317236l;

    /* renamed from: m, reason: collision with root package name */
    public final /* synthetic */ RecyclerView f317237m;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public c(d dVar, RecyclerView recyclerView) {
        super(1);
        this.f317236l = dVar;
        this.f317237m = recyclerView;
    }

    @Override // Y41.l
    public final G0 invoke(String str) {
        String str2 = str;
        d dVar = this.f317236l;
        dVar.f317247j = str2;
        dVar.f317239b.c(new UV0.c(d.d(str2, dVar.f317243f)));
        this.f317237m.A0(0);
        return G0.f406611a;
    }
}
