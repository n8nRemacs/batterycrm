package com.avito.android.iac_dialer.impl_module.telecom;

import Y61.k;
import Y61.l;
import com.avito.android.iac_dialer.impl_module.active_call_processing.iac_dialer.logic.state.call_state.TerminateReasonBundle;
import com.avito.android.iac_dialer.impl_module.audio.audio_devices.AudioDevice;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.Z;
import kotlin.coroutines.Continuation;

/* compiled from: IacTelecomManager.kt */
@Metadata(d1 = {"\u0000\n\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0000\bf\u0018\u00002\u00020\u0001¨\u0006\u0002"}, d2 = {"Lcom/avito/android/iac_dialer/impl_module/telecom/a;", "", "_avito_iac-dialer_impl"}, k = 1, mv = {1, 9, 0}, xi = 48)
/* loaded from: classes14.dex */
public interface a {
    @l
    Object a(@k Continuation<? super Z<G0>> continuation);

    void b(@k TerminateReasonBundle terminateReasonBundle);

    boolean c();

    void d(@k AudioDevice audioDevice);

    @l
    Object e(boolean z12, @k Continuation<? super Z<G0>> continuation);
}
