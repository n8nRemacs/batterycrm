package com.avito.android.entry_point_client_room.report;

import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;
import sz.AbstractC48438c;

/* compiled from: SendReportScreen.kt */
@Metadata(d1 = {"\u0000\f\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0004\u001a\u00020\u0001*\u00020\u0000H\n¢\u0006\u0004\b\u0002\u0010\u0003"}, d2 = {"Lcom/avito/android/lib/design/text_view/a;", "Lkotlin/G0;", "invoke", "(Lcom/avito/android/lib/design/text_view/a;)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes13.dex */
final class l extends N implements Y41.l<com.avito.android.lib.design.text_view.a, G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ sz.e f147691l;

    /* renamed from: m, reason: collision with root package name */
    public final /* synthetic */ Y41.l<AbstractC48438c, G0> f147692m;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    /* JADX WARN: Multi-variable type inference failed */
    public l(sz.e eVar, Y41.l<? super AbstractC48438c, G0> lVar) {
        super(1);
        this.f147691l = eVar;
        this.f147692m = lVar;
    }

    @Override // Y41.l
    public final G0 invoke(com.avito.android.lib.design.text_view.a aVar) {
        this.f147691l.f438955g.setOnDeepLinkClickListener(new SE0.a(23, this.f147692m));
        return G0.f406611a;
    }
}
