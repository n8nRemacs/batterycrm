package com.avito.android.extended_profile_universal_widget_edit.edit_block;

import com.avito.android.extended_profile_universal_widget_edit.edit_block.UniversalWidgetEditBlockFragment;
import kB.InterfaceC42561a;
import kotlin.G0;
import kotlin.Metadata;
import kotlin.jvm.internal.N;

/* compiled from: UniversalWidgetEditBlockView.kt */
@Metadata(d1 = {"\u0000\u000e\n\u0002\u0010\u000e\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0005\u001a\u00020\u00022\u0006\u0010\u0001\u001a\u00020\u0000H\n¢\u0006\u0004\b\u0003\u0010\u0004"}, d2 = {"", "text", "Lkotlin/G0;", "invoke", "(Ljava/lang/String;)V", "<anonymous>"}, k = 3, mv = {1, 9, 0})
/* loaded from: classes13.dex */
final class j extends N implements Y41.l<String, G0> {

    /* renamed from: l, reason: collision with root package name */
    public final /* synthetic */ i f154075l;

    /* renamed from: m, reason: collision with root package name */
    public final /* synthetic */ int f154076m;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public j(i iVar, int i12) {
        super(1);
        this.f154075l = iVar;
        this.f154076m = i12;
    }

    @Override // Y41.l
    public final G0 invoke(String str) {
        Y41.l<InterfaceC42561a, G0> lVar = this.f154075l.f154056b;
        UniversalWidgetEditBlockFragment.e eVar = (UniversalWidgetEditBlockFragment.e) lVar;
        eVar.invoke(new InterfaceC42561a.h(this.f154076m, str));
        return G0.f406611a;
    }
}
