...
public final class RetryAndFollowUpInterceptor implements Interceptor {
    ...
    public RetryAndFollowUpInterceptor(@k OkHttpClient okHttpClient) {
    ...
    private final Request buildRedirectRequest(Response userResponse, String method) {
        ...
        if (!L.f(httpUrlResolve.scheme(), userResponse.request().url().scheme()) && !this.client.followSslRedirects()) {
            return null;
        }
        ...
        if (HttpMethod.permitsRequestBody(method)) {
            ...
            boolean z12 = httpMethod.redirectsWithBody(method) || iCode == 308 || iCode == 307;
            if (!httpMethod.redirectsToGet(method) || iCode == 308 || iCode == 307) {
                builderNewBuilder.method(method, z12 ? userResponse.request().body() : null);
            } else {
    ...
    private final boolean isRecoverable(IOException e12, boolean requestSendStarted) {
    ...
    private final boolean recover(IOException e12, RealCall call, Request userRequest, boolean requestSendStarted) {
        if (this.client.retryOnConnectionFailure()) {
            return !(requestSendStarted && requestIsOneShot(e12, userRequest)) && isRecoverable(e12, requestSendStarted) && call.retryAfterFailure();
        }
        return false;
    }

    private final boolean requestIsOneShot(IOException e12, Request userRequest) {
        RequestBody requestBodyBody = userRequest.body();
        return (requestBodyBody != null && requestBodyBody.isOneShot()) || (e12 instanceof FileNotFoundException);
    }
    ...
    private final int retryAfter(Response userResponse, int defaultDelay) {
        ...
        if (new C43059p(InlineFilterValue.InlineFilterDateRangeValue.NUMERIC_ID_REGEX_PATTERN).e(strHeader$default)) {
    ...
    public Response intercept(@k Interceptor.Chain chain) {
        ...
        while (true) {
            call.enterNetworkInterceptorExchange(request, z12);
            try {
                if (call.getCanceled()) {
                ...
                try {
                    ...
                    if (!recover(e12, call, request, !(e12 instanceof ConnectionShutdownException))) {
                        throw Util.withSuppressed(e12, listI0);
                    }
                    listI0 = C42745f0.i0(e12, listI0);
                    call.exitNetworkInterceptorExchange$okhttp(true);
                    z12 = false;
                } catch (RouteException e13) {
                    if (!recover(e13.getLastConnectException(), call, request, false)) {
                        throw Util.withSuppressed(e13.getFirstConnectException(), listI0);
                    }
                    listI0 = C42745f0.i0(e13.getFirstConnectException(), listI0);
                    call.exitNetworkInterceptorExchange$okhttp(true);
                    z12 = false;
                }
